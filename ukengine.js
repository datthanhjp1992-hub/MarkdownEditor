...
(function (root, factory) {
  ...
  function buildDT(method, bracketShortcuts) {
    var dt = Object.create(null), i, j, ch;
    if (bracketShortcuts === undefined) bracketShortcuts = true;
    ...
    if (method === TELEX) {
      ...
      for (i = 0; i < TELEX_SHORTCUTS.length; i++) {
        ...
        // Khi tắt bracketShortcuts: chỉ giữ phím w (w đứng một mình -> ư),
        // còn [ ] { } trả về đúng ký tự gốc — cần thiết cho cú pháp Markdown
        // như [link](url) hay checklist - [ ].
        if (!bracketShortcuts && ch !== 'w') continue;
  ...
  function UkEngine(opts) {
    ...
    this.options = {
      ...
      // Phím tắt [ ] { } -> ơ ư. Nên đặt false trong trình soạn thảo Markdown/code.
      bracketShortcuts: opts.bracketShortcuts !== false
    };
    this.macros = new Map();
    this.enabled = true;  // false = tắt bộ gõ, phím đi thẳng vào ô nhập
    this.dirty = true;    // true = buffer có thể đã lệch với ô nhập, cần nạp lại
  ...
  P.setMethod = function (m) {
    this.method = m;
    this.DT = buildDT(m, this.options.bracketShortcuts);
    this.clearBuf();
  };

  P.setOption = function (k, v) {
    this.options[k] = v;
    if (k === 'bracketShortcuts') this.setMethod(this.method);
  };

  /** Báo cho engine biết nội dung ô nhập vừa bị thay đổi từ bên ngoài
   *  (toolbar, import, undo, paste...) -> lần gõ kế tiếp sẽ nạp lại buffer. */
  P.markDirty = function () { this.dirty = true; };
  ...
  /** Kiểm tra buffer có còn khớp với văn bản thật đứng trước con trỏ không.
   *  Đây là "lưới an toàn": mọi thao tác sửa nội dung từ bên ngoài mà quên báo
   *  markDirty() vẫn bị phát hiện ở đây, tránh engine xoá nhầm ký tự (backs). */
  P.matchesTail = function (textBeforeCaret) {
    var n = this.buf.length;
    if (n === 0) return true;
    if (textBeforeCaret.length < n) return false;
    var tail = textBeforeCaret.slice(textBeforeCaret.length - n).toLowerCase();
    for (var i = 0; i < n; i++) if (tail[i] !== this.buf[i]) return false;
    return true;
  };
  ...
  UkEngine.attach = function (el, engine) {
    // Số ký tự đứng trước con trỏ cần đọc lại khi nạp buffer. Buffer tối đa
    // KEY_BUFSIZE ký tự nên đọc dư một chút là đủ; tránh cắt cả tài liệu dài.
    var SYNC_LOOKBEHIND = KEY_BUFSIZE + 20;

    var composing = false;   // đang có IME khác (tiếng Nhật, tiếng Trung...) hoạt động

    var isPrintable = function (e) {
      return e.key && e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey;
    };

    // Đang trong phiên soạn thảo của IME hệ điều hành. Chrome/Edge báo keyCode 229,
    // chuẩn DOM báo isComposing. Bắt buộc phải bỏ qua, nếu không sẽ vừa preventDefault
    // vừa chèn tay -> ký tự bị nhân đôi hoặc IME chết cứng.
    var isComposingKey = function (e) {
    ...
    function insert(backs, text) {
      ...
      // Ưu tiên execCommand('insertText'): đây là cách duy nhất ghi được vào
      // undo stack của trình duyệt, nhờ vậy Ctrl+Z / Ctrl+Y vẫn hoạt động bình thường.
      try {
      ...
      if (!ok) {   // trình duyệt không hỗ trợ -> quay về setRangeText
    ...
    function onKeyDown(ev) {
      ...
      if (isComposingKey(ev)) {          // nhường quyền cho IME của hệ điều hành
        engine.clearBuf();
        engine.dirty = true;
        return;
      }
      if (ev.key === 'Backspace' && ev.target === el) {
        if (el.selectionStart !== el.selectionEnd) { engine.clearBuf(); engine.dirty = true; }
      ...
      if (ev.key === 'Enter' || ev.key === 'Tab') {
        engine.clearBuf();
        engine.dirty = true;             // Enter có thể bị code khác chèn thêm nội dung
        return;
      }
      // Ctrl/Cmd + phím: undo, redo, paste, phím tắt của app... đều đổi nội dung
      if (ev.ctrlKey || ev.metaKey || ev.altKey) { engine.dirty = true; return; }
      if (!isPrintable(ev)) return;
      if (el.selectionStart !== el.selectionEnd) engine.clearBuf();

      var caret = el.selectionStart;
      var before = el.value.slice(Math.max(0, caret - SYNC_LOOKBEHIND), caret);
      // Buffer lệch với nội dung thật (toolbar chèn, undo, paste...) → nạp lại
      if (!engine.dirty && !engine.matchesTail(before)) engine.dirty = true;
      if (engine.dirty) {
        engine.syncFrom(before);
        engine.dirty = false;
      }
    ...
    function onBlur() { engine.clearBuf(); engine.dirty = true; }
    function onCompStart() { composing = true; engine.clearBuf(); }
    function onCompEnd() { composing = false; engine.dirty = true; }

    // Cố ý KHÔNG lắng nghe 'select': hàm insert() ở trên có gọi setSelectionRange
    // nên sự kiện này sẽ tự kích hoạt và làm mất trạng thái gõ lại (aaa, ww...).
    // Các trường hợp còn lại đã được matchesTail() bắt.
    var dirtyEvents = ['mousedown', 'focus', 'paste', 'cut', 'drop'];
    el.addEventListener('keydown', onKeyDown);
    dirtyEvents.forEach(function (n) { el.addEventListener(n, onDirty); });
    ...
    return function detach() {
      ...
      dirtyEvents.forEach(function (n) { el.removeEventListener(n, onDirty); });
